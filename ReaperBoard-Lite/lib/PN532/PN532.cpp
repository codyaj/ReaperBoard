...
*/
/**************************************************************************/
bool PN532::inListPassiveTarget(uint8_t *atqa, uint8_t *sak, uint8_t *uid, uint8_t *uidLength) { // ? Modified for ReaperBoard
    pn532_packetbuffer[0] = PN532_COMMAND_INLISTPASSIVETARGET;
    pn532_packetbuffer[1] = 1;  // max targets
    pn532_packetbuffer[2] = 0;  // baud rate

    if (HAL(writeCommand)(pn532_packetbuffer, 3)) {
        return false;
    }

    int16_t status = HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer), 10); // approx 200ms delay
    if (status < 0) {
        return false;
    }
    ...
    // Parse the response buffer according to PN532 datasheet
    // response bytes after pn532_packetbuffer[1]:
    // [1] = target number (0x01)
    // [2,3] = ATQA (2 bytes)
    // [4] = UID length
    // [5..] = UID bytes
    // After UID = SAK

    uint8_t target = pn532_packetbuffer[1];
    atqa[0] = pn532_packetbuffer[2];
    atqa[1] = pn532_packetbuffer[3];

    *uidLength = pn532_packetbuffer[4];
    for (uint8_t i = 0; i < *uidLength; i++) {
        uid[i] = pn532_packetbuffer[5 + i];
    }

    *sak = pn532_packetbuffer[5 + *uidLength];
